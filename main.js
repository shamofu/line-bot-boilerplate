...
const reminder = new agenda({
  db: {address: process.env.MONGODB_URI, collection: 'reminder'}
})
...
const handleMessages = async event => {
  ...
  const [userId, displayName] = await (async event => {
    ...
    switch (event.source.type) {
      ...
      case 'room': {
        const profile = await client
          .getRoomMemberProfile(event.source.roomId, event.source.userId)
          .catch(() => dummyProfile)
        return [profile.userId, profile.displayName]
      }
      default: {
  ...
  switch (command[1]) {
    ...
    case 'remind': {
      ...
      const reply = {
        type: 'text',
        text: `I'll remind you ${moment(time).format('YYYY-MM-DD HH:mm:ss Z')}.`
      }
